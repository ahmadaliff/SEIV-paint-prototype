...
const DISTRICTS = [
  ...
  { name: "Serpong, Tangerang", lat: -6.3039, lng: 106.666 },
  { name: "Cikarang, Bekasi", lat: -6.2891, lng: 107.1425 },
  { name: "Pasteur, Bandung", lat: -6.8906, lng: 107.596 },
...
export default function CheckoutPage() {
  ...
  const matchedZone = shippingZones.find((z) =>
    z.name.includes(targetZoneName),
  );
  ...
  if (orderSuccess) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6 text-center">
        ...
        <h1 className="text-3xl font-black text-slate-900 mb-2 font-serif">
          Pesanan Berhasil!
        </h1>
        <p className="text-slate-500 mb-8 max-w-md">
          Terima kasih sudah berbelanja di SEIV Paint. Pesanan Anda{" "}
          <strong>#{orderSuccess.id}</strong> sedang kami proses.
        </p>
        ...
        <div className="bg-slate-50 p-6 rounded-2xl w-full max-w-sm border border-slate-100 mb-8">
          <div className="flex justify-between text-sm mb-2">
            ...
            <span className="font-bold text-slate-900">
              {formatRupiah(orderSuccess.summary.current_order_total)}
            </span>
          </div>
        ...
        <Link
          href="/admin"
          className="bg-[#1a2b43] text-white px-8 py-3 rounded-xl font-bold transition-all hover:bg-slate-800 shadow-xl"
        >
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-24">
      ...
      <main className="mx-auto max-w-5xl px-6 py-10">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-2xl font-black text-[#1a2b43] font-serif">
            Checkout
          </h1>
          {isLoadingCart && (
        ...
        <div className="flex items-center gap-4 mb-10 text-[10px] uppercase tracking-widest font-black w-full max-w-2xl mx-auto">
          <div
            className={`flex items-center gap-2 ${step >= 1 ? "text-[#1a2b43]" : "text-slate-300"}`}
          >
            <span
              className={`flex h-10 w-10 items-center justify-center rounded-full ${step >= 1 ? "bg-[#1a2b43] text-white" : "bg-slate-200 text-slate-400"}`}
            >
              1
            </span>
            Alamat
          ...
          <div
            className={`h-1 w-25 rounded-full ${step >= 2 ? "bg-[#1a2b43]" : "bg-slate-200"}`}
          ></div>
          <div
            className={`flex items-center gap-2 ${step >= 2 ? "text-[#1a2b43]" : "text-slate-300"}`}
          >
            <span
              className={`flex h-10 w-10 items-center justify-center rounded-full ${step >= 2 ? "bg-[#1a2b43] text-white" : "bg-slate-200 text-slate-400"}`}
            >
              2
            </span>
            Review
          ...
          <div
            className={`h-1 w-25 rounded-full ${step >= 3 ? "bg-[#1a2b43]" : "bg-slate-200"}`}
          ></div>
          <div
            className={`flex items-center gap-2 ${step >= 3 ? "text-[#1a2b43]" : "text-slate-300"}`}
          >
            <span
              className={`flex h-10 w-10 items-center justify-center rounded-full ${step >= 3 ? "bg-[#1a2b43] text-white" : "bg-slate-200 text-slate-400"}`}
            >
              3
            </span>
            Pembayaran
        ...
        <div className="grid grid-cols-2 gap-8">
                  ...
                  <div className="space-y-4 text-black">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full border border-slate-200 rounded-lg p-3 text-sm focus:ring-2 focus:ring-blue-500 bg-slate-50"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] font-black uppercase tracking-widest mb-1.5">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="w-full border border-slate-200 rounded-lg p-3 text-sm focus:ring-2 focus:ring-blue-500 bg-white shadow-sm"
                      >
                        {DISTRICTS.map((d) => (
                    ...
                    <div>
                      ...
                      <textarea
                        defaultValue="Jl. Sudirman No. 45, RT 02/RW 05"
                        rows={2}
                        className="w-full border border-slate-200 rounded-lg p-3 text-sm focus:ring-2 focus:ring-blue-500 bg-slate-50"
                      />
                    </div>
                    <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 shadow-sm">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <h4 className="text-xs font-black text-blue-800 uppercase tracking-widest font semi-bold">
                            Haversine Engine (Medusa V2)
                          </h4>
                          <p className="text-xs text-blue-700 mt-1">
                            Dihitung dari {district.name} &rarr;{" "}
                            <strong>{nearestBranch.name}</strong>:{" "}
                    ...
                    <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 shadow-sm flex flex-col gap-4">
                      <div className="flex gap-2">
                        <Package className="h-5 w-5 text-brown-500 shrink-0" />
                        <h3 className="text-md font-black text-blue-800 font-semibold">
                          Info Pengiriman Terdekat
                        </h3>
                      </div>
                      <div className="flex flex-col gap-2">
                        <span className="flex gap-1 text-xs items-center">
                          ...
                          Cabang :
                          <div className="font-bold">{nearestBranch.name}</div>
                        </span>
                        <span className="flex gap-1 text-xs items-center">
                          <Ruler className="text-yellow-600 h-4 w-4" />
                          Jarak :
                          <div className="font-bold">{minDistance} Km</div>
                        </span>
                        <span className="flex gap-1 text-xs items-center">
                      ...
                      <div className="space-y-3 relative z-10">
                        <label className="flex items-start gap-3 p-4 border-2 border-black bg-white rounded-xl cursor-pointer transition-all">
                          <input
                            type="radio"
                            name="shipping"
                            className="mt-1"
                          ...
                          <div className="flex-1">
                            <div className="flex justify-between items-center">
                              <span className="font-bold text-sm">
                                SEIV Express - {targetZoneName}
                                <p className="text-[10px] text-slate-400 mt-1 uppercase font-bold tracking-tighter">
                                  Estimasi:{" "}
                                  {minDistance < 50 ? "Same-Day" : "Reguler"}
                                </p>
                              </span>
                              <span className="font-black text-md text-blue-700">
                                {matchedZone?.price === 0
                                  ? "GRATIS"
                                  : formatRupiah(matchedZone?.price || 0)}
                              </span>
                            </div>
                          </div>
                        </label>
                  ...
                  <div className="grid grid-cols-2 gap-8">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1 leading-relaxed">
                        Jl. Sudirman No. 45, {district.name}
                      </p>
                    </div>
                    <div>
                      <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">
                      ...
                      <p className="text-[10px] text-blue-600 font-bold mt-1 uppercase">
                        Estimasi: {minDistance < 50 ? "Tiba Hari Ini" : "Besok"}
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="bg-slate-50 rounded-2xl p-4 divide-y divide-slate-200 border border-slate-100">
                      {fullCart?.items.map((item: any) => (
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <h5 className="text-xs font-bold text-slate-800 uppercase tracking-tight line-clamp-1">
                              {item.title}
                            </h5>
              ...
              <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="space-y-4">
                  {[
                    {
                      id: "va",
                      name: "Virtual Account (BCA/Mandiri)",
                      icon: "💳",
                    },
                    {
                      id: "qris",
          ...
          <div className="lg:col-span-2">
            <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-xl shadow-slate-200/50 sticky top-24 overflow-hidden">
              <h2 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-6 relative z-10">
                Ringkasan Pesanan
              </h2>
